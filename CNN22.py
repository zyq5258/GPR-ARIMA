...
from tensorflow.keras.layers import Dropout, Input, Conv1D, Flatten, MaxPooling1D
...
model = Sequential()
model.add(Conv1D(filters=8, kernel_size=3, activation='relu', input_shape=(trainX.shape[1], trainX.shape[-1])))
...
train_t0 = time()
model.fit(trainX, trainY, epochs=1, batch_size=50, verbose=1)
train_t1 = time()
...
np.savetxt('cnn predict result.csv',
...
cnn_mse = np.zeros([len(testPredict), 1], dtype=float)  # 均方误差
cnn_rmse = np.zeros([len(testPredict), 1], dtype=float)  # 均方根误差
cnn_mae = np.zeros([len(testPredict), 1], dtype=float)  # 平均绝对误差
cnn_mape = np.zeros([len(testPredict), 1], dtype=float)  # 平均绝对百分误差
cnn_r2_score = np.zeros([len(testPredict), 1], dtype=float)  # 决定系数,越大于好，最大为1

for i in range(int(len(df) * 0.7) + 1, int(len(df)) + 1, 1):
    print(i)
    cnn_mse[i - 1187] = mean_squared_error(testY.loc[int(len(df) * 0.7) + 1:i],
                                           testPredict.loc[int(len(df) * 0.7) + 1:i])  # 均方误差

    cnn_rmse[i - 1187] = sqrt(mean_squared_error(testY.loc[int(len(df) * 0.7) + 1:i],
                                                 testPredict.loc[int(len(df) * 0.7) + 1:i]))  # 均方根误差

    cnn_mae[i - 1187] = mean_absolute_error(testY.loc[int(len(df) * 0.7) + 1:i],
                                            testPredict.loc[int(len(df) * 0.7) + 1:i])  # 平均绝对误差

    cnn_mape[i - 1187] = mean_absolute_percentage_error(testY.loc[int(len(df) * 0.7) + 1:i],
                                                        testPredict.loc[int(len(df) * 0.7) + 1:i])  # 平均绝对百分误差

    cnn_r2_score[i - 1187] = r2_score(testY.loc[int(len(df) * 0.7) + 1:i],
                                      testPredict.loc[int(len(df) * 0.7) + 1:i])  # 决定系数

"保存结果"
np.savetxt('cnn predict performance.csv',
           np.column_stack((result_index,
                            cnn_mse,
                            cnn_rmse,
                            cnn_mae,
...
plt.figure()
plt.title("cnn_mse")
plt.plot(cnn_mse[0:30], label="cnn_mse")
plt.legend()
plt.show()

plt.figure()
plt.title("cnn_rmse")
plt.plot(cnn_rmse[0:30], label="cnn_rmse")
plt.legend()
plt.show()

plt.figure()
plt.title("cnn_mae")
plt.plot(cnn_mae[0:30], label="cnn_mae")
plt.legend()
plt.show()

plt.figure()
plt.title("cnn_mape")
plt.plot(cnn_mape[0:30], label="cnn_mape")
plt.legend()
plt.show()

plt.figure()
plt.title("cnn_r2_score")
plt.plot(cnn_r2_score[0:30], label="cnn_r2_score")
plt.legend()
plt.show()
